...
rem 建立文件名前缀
set filename_prefix=InteractLiveroom_SRC
...
set test=rar a -r -phuashanlj -ed  -x*.VC.db -x*.tlog -x*.log -xtmp*/ -x.git -x*.sdf -x*.pdb -x*.pch -x*.exp -x*.ilk -x*_d.exe -x*.zip -x*.rar -x*\x64*\* -x*\x86_dlls\* -x*.ipch -x*.obj %rar_filename% .\*.*
%test%                  
pause