...
void VideoSettingForm::InitWindow()
{
	...
	open_beauty_checkbox_ = (CheckBox*)FindControl(L"open_beauty");
}
...
bool VideoSettingForm::OnSelect( ui::EventArgs* arg )
{
	...
	else if (name == L"open_beauty")
	{
		bool select = open_beauty_checkbox_->IsSelected();
		if (select)
		{
			VideoManager::GetInstance()->video_frame_mng_.SetBeautyOpt(true);
		}
		else
		{
			VideoManager::GetInstance()->video_frame_mng_.SetBeautyOpt(false);
		}
	}
	return true;
}
...
void VideoSettingForm::ShowPage( bool video )
{
	...
	if(video)
	{
		open_beauty_checkbox_->Selected(VideoManager::GetInstance()->video_frame_mng_.GetBeautyOptStatus(), false);
		VideoManager::GetInstance()->EndDevice(nim::kNIMDeviceTypeAudioIn, kDeviceSessionTypeSetting);
