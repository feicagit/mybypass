...
void CaptureWindow::SetSendInfo(std::wstring send_info)
{
	...
	if(multilan)
	{
		...
		nbase::StringPrintf(send_info_temp, multilan->GetStringViaID(L"STRING_CAPTUREFORM_SENDINFO").c_str(), send_info.c_str());
...
void CaptureWindow::InitWindow()
{
	...
	if (hUser32)
	{
		...
		if (pSetProcessDPIAware)
		{
			...
			//LOG_APP(L"hPixelsPerInch desk %d", hPixelsPerInch);
			//ReleaseDC(NULL, screen);
			//screen = GetDC(GetHWND());
			//LOG_APP(L"hPixelsPerInch cur %d", hPixelsPerInch);
			...
			else
			{
				...
				{
					//LOG_APP(L"applied_dpi %d", data);
					if (data > 0)
					{
						desktop_scale_ = data * 100 / 96;
					}
					::RegCloseKey(hKEY);
				}
			}
	...
	//LOG_APP(L"capture info screen x %d,y %d,w %d,h %d,desktop_scale %d", screen_x_, screen_y_, screen_width_, screen_height_, desktop_scale_);
	...
	{                   
		...
		if(multilan)
		{
			text->SetText(multilan->GetStringViaID(L"STRING_CAPTUREFORM_SIZEINFO").c_str());
		}
		else
		{
	...
	SetPos(ui::UiRect(screen_x_, screen_y_, screen_x_ + screen_width_, screen_y_+screen_height_), SWP_NOACTIVATE, NULL);
#else
...
RECT CaptureWindow::GetToolBarRect()
{
	...
	if(is_show_color_palette_)
	{
		...
		color_palette_->ShowRange(capture_edit_mode_ == EM_MOSAIC || capture_edit_mode_ == EM_GAUSS);
...
void CaptureWindow::ShowToolBar(bool edit)
{
	...
	if (control2 && send_info_.size() > 0)
	{
		...
		if (show_toolbar)
		{
			...
			control2->SetMargin(margin);
			control2->SetFixedWidth(track_board.right - track_board.left);
			control2->SetFixedHeight(track_board.bottom - track_board.top);
...
void CaptureWindow::Save()
{
	...
	nbase::StringPrintf(file_name, L"截图%d%d%d%d%d%d",
