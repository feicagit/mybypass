...
//////////////////////////////////文字绘制类////////////////////////////////////////
DrawUnitText::DrawUnitText(int x,int y,const RECT& rcValid, DWORD font_color, BOOL is_generated)
:DrawUnit(x,y,rcValid)
{
	is_text_has_generated_	=	is_generated;
	font_color_				=	font_color;
	is_font_italic_			=	FALSE;
	is_font_underline_		=	FALSE;
	font_height_			=	knDefaultFontSize; // 18px（与实际有偏差）
	is_bold_				=	FALSE;
}
...
void DrawUnitText::DrawRect(HDC hdc)
{
	...
	// 虚线画笔
	HPEN dot_pen = CreatePen(PS_DOT, 1, font_color_);
...
void DrawUnitText::DrawText(HDC hdc)
{
	// 使得文本背景透明，与窗口背景一样
	int old_bk_mode = SetBkMode(hdc, TRANSPARENT); 
	COLORREF old_color = ::SetTextColor(hdc, font_color_);
	HFONT font = CreateFont(
		-1 * ROUND(font_height_* 1.33333), // nHeight
		0,								// nWidth
		0,								// nEscapement
		0,								// nOrientation
		is_bold_ ? FW_BOLD :FW_NORMAL , // nWeight
		is_font_italic_,				// bItalic
		is_font_underline_,				// bUnderline
		0,								// cStrikeOut
