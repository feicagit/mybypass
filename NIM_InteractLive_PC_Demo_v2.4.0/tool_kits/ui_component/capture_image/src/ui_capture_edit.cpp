...
void CCaptureEdit::Paint( HDC hDC, const ui::UiRect& rcPaint )
{
...
{
	if (::PtInRect(&track_rect_, point) && text_edit_ == NULL)
	{
...
void CCaptureEdit::OnDrawUnitEnd( const ui::CPoint &point)
{
	...
	if (edit_mode_ == EM_TEXT)	//文本需要额外处理
	{
		...
		{
			// 确定文本编辑框的rect
			ui::UiRect rc_edit = current_draw_unit_->GetWindowRect(); 
			text_edit_ = new CCaptureTextEdit(rc_edit);
			text_edit_->SetFixedHeight(rc_edit.GetHeight());
			text_edit_->SetFixedWidth(rc_edit.GetWidth());
			text_edit_->SetFloat(true);
			text_edit_->SetAttribute(L"multiline", L"true");
			text_edit_->SetAttribute(L"returnmsgwantctrl", L"true");
			text_edit_->SetAttribute(L"align", L"left");
			if(color_palette_)
			{
				//DWORD text_color = color_palette_->GetSelectedColorRGB();
				//BYTE R = GetRValue(text_color);
				//BYTE G = GetGValue(text_color);
				//BYTE B = GetBValue(text_color);
				//text_edit_->SetTextColor(RGB(B, G, R)); // RichEdit
				//text_edit_->SetDotPenColor(text_color); // 虚线框的颜色
			}
			text_edit_->SetFont(12); // 默认字体14px
			ui::Box* parent_hor = static_cast<ui::Box*>(this->GetParent());
			RECT margin = {rc_edit.left - parent_hor->GetPos().left, 
				rc_edit.top - parent_hor->GetPos().top, 0, 0};
			text_edit_->SetMargin(margin);
...
void CCaptureEdit::CreateDrawUnit( const ui::CPoint& point)
{
	...
	{
	case EM_TEXT:
		current_draw_unit_ = new DrawUnitText(point.x, point.y, track_rect_,
			color_palette_->GetSelectedColorRGB(), FALSE);
		break;
	case EM_RECTANGLE:
...
void CCaptureEdit::OnTextEditFinished()
{
	...
	current_draw_unit_ = new DrawUnitText(edit_rect.left, edit_rect.top, track_rect_, text_color, TRUE); // 绘制文本
...
//#endif
	...
	if (range > 0)
	{
		...
		case DrawUnit::BLUR_TYPE_GAUSS:
		{
			...
			ImageEffects::GaussianBlur(4, sse_check, false, 0, 0, width, height, width * 4, range, (unsigned int*)pBitmapData, (unsigned int*)pBitmapData);
		}
			break;
		}
	}
